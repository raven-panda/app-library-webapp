      ...
      <section>
        {CollectionUtils.isEmpty(books) ? (
          <h1>
            ...
            <button onClick={() => setSearchParams()} className="link">
              {t('browsePage.clearSearch')}
            </button>
          </h1>
        ) : (
          <>
            ...
            <div className="ebr_browse-page-content">
              {books?.map((book) => (
    ...
    <Link to={`/book/${book.id}`}>
      <article className="ebr_book-card">
        <img
          ...
          height={180}
          alt={book.title}
          src={API_FILE.replace(':id', book.coverFileId)}
        />
        <div>
          <h2>{book.title}</h2>
          <p>
            <span className="ebr_book-card-prop">{t('author')} :</span>{' '}
            {book.authorFullName}
          </p>
          <p>
            <span className="ebr_book-card-prop">{t('editor')} :</span>{' '}
            {book.editor}
          </p>
          <p>
            <span className="ebr_book-card-prop">{t('note')} :</span>{' '}
            {book.averageRate} ({book.reviews} {t('reviews')})
          </p>
          <p>
            <span className="ebr_book-card-prop">{t('price')} :</span>{' '}
            {book.price}€
          </p>
        </div>
      </article>
